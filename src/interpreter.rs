  ...
  pub fn exec(&self, genv: &mut EnvObj<Entry>, env: &mut Obj, obj: &mut Obj) -> Result<()> {
    ...
    match *self {
      ...
      SlotStmt::Method(ref met) => {
        env_obj.borrow_mut().add(&met.name[..], Entry::Func(met.body.clone(), met.args.clone()));
      }
    }
    Ok(())
  }
  ...
  pub fn eval(&self, genv: &mut EnvObj<Entry>, env: &mut Obj) -> Result<Obj> {
    match *self {
      ...
      ScopeStmt::Fn(ref fun) => {
        ...
        genv.add(&fun.name[..], Entry::Func(fun.body.clone(), fun.args.clone()));
        Ok(Obj::Null)
      }
...
mod test_int_obj {
  ...
  #[test]
  fn test_number_ops() {
    assert_eq!(IntObj { value: 2 } + IntObj { value: 3 }, IntObj { value: 5 });
    assert_eq!(IntObj { value: 2 } - IntObj { value: 3 }, IntObj { value: -1 });
    assert_eq!(IntObj { value: 2 } * IntObj { value: 3 }, IntObj { value: 6 });
  }
}
