    ...
    pub fn compile(
        ...
        match *self {
            ...
            Exp::SetSlot(ref setslot) => {
                debug!(
                    "Setting slot: name: {} exp: {:?} value: {:?}",
                    setslot.name, setslot.exp, setslot.value
                );
            ...
            Exp::CallSlot(ref callslot) => {
                debug!(
                    "Calling slot: name: {} exp: {:?} args: {:?}",
                    callslot.name, callslot.exp, callslot.args
                );
            ...
            Exp::If(ref if_exp) => {
                debug!(
                    ...
                    if_exp.pred, if_exp.conseq, if_exp.alt
                );
    ...
    pub fn compile(
        ...
        match *self {
            ...
            ScopeStmt::Fn(ref fun) => {
                debug!(
                    "Scope Fn: name: {:?} args: {:?} body: {:?}",
                    fun.name, fun.args, fun.body
                );
    ...
    pub fn compile(
        ...
        match *self {
            ...
            SlotStmt::Method(ref met) => {
                debug!(
                    "Slot method: name: {:?} args: {:?} body: {:?}",
                    met.name, met.args, met.body
                );
